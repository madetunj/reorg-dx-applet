...
def main(): 
    ...
    analysis_id = "analysis-Fx2244Q95JGFQJYGFJXfQpYJ"
...
#ROSE
    ...
    for thestring in (pngfile, enhancers, super_enhancers, union_enhancers, stitch_enhancers, e_to_g_enhancers, e_to_g_super_enhancers, g_to_e_enhancers, g_to_e_super_enhancers):
        if thestring is not None :
            dx_container.move(
...
#MOTIFS
    ...
    summit_motif_subfile = 'bklist' + (dxpy.describe(flankbedfile)["name"].split('bklist')[1]).split('.bed')[0]
    motif_subfile = 'bklist' + (dxpy.describe(peakbedfile)["name"].split('bklist')[1]).split('.bed')[0]
...
#BAM2GFF
    ...
    for thestring in (pdf_gene, pdf_h_gene, png_h_gene, pdf_promoters, pdf_h_promoters, png_h_promoters):
        if thestring is not None :
            dx_container.move(
                destination=prefolder,
                objects=[ thestring['$dnanexus_link'] ]
...
#VISUALIZATION
    ...
    dx_container.new_folder(folder,parents=True)
    for thestring in (bigwig, norm_wig, tdffile, a_bigwig, a_norm_wig, a_tdffile, n_bigwig, n_norm_wig, n_tdffile):
        if thestring is not None :
            dx_container.move(
                destination=folder,
                objects=[ thestring['$dnanexus_link'] ]
